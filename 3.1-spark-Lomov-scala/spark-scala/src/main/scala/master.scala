...
var rows = dataStrings.map(_.split(",")).map(attributes => Row(
...
// Create rows with new column
rows = dataStrings.map(_.split(",")).map(attributes => Row(
  attributes(0).trim().toInt,
  {
    val strTimeStamp = attributes(1).trim()
    var t = strTimeStamp.toLong * 1000
  ...
  attributes(5).trim().toBoolean,
  {
    ...
    //          val ranges = List((0, 4), (4, 8), (8, 12), (12, 16), (20, 0)) // 0-4
    var hh = ttt.getHour
    if (0 <= ttt.getHour && ttt.getHour < 4)  ranges(0).toString()
    else if (4 <= ttt.getHour && ttt.getHour < 8)  ranges(1).toString()
    else if (8 <= ttt.getHour && ttt.getHour < 12)  ranges(2).toString()
    else if (12 <= ttt.getHour && ttt.getHour < 16)  ranges(3).toString()
    else if (20 <= ttt.getHour && ttt.getHour < 0)  ranges(4).toString()
  }
))
...
var lkdf = spark.createDataFrame(
  spark.sparkContext.parallelize(lkRows),
  schema
)
